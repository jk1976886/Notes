...
\begin{document}
    ...
    Consider\\
    $$max\{c^Tx:Ax = b, x \geq 0\}$$
    To run Simplex, we need a \red{feasible} basis. So how do we find a feasible basis?\\
    Well, an easier question to answer is: How do we find a feasible solution?\\
    Turns out that the two questions above are equivalent, so we answer the second one.\\
    \bigskip
    \textbf{Key Idea:}\\
    Given 
    $$max\{c^Tx:Ax = b, x \geq 0\}$$
    We have 2 algorithms:\\
    \fbox{\parbox{\linewidth}{{\red{Algorithm 1:}\\
    \textbf{Input: } $A, b, c$ and a feasible solution\\
    \textbf{Output: } Optimal solution/detect LP is unbounded.\\}}}\\
    \bigskip
    \fbox{\parbox{\linewidth}{{\red{Algorithm 2:}\\
    \textbf{Input: } $A$ and $b$\\
    \textbf{Output: } Feasible solution/detect there is no feasible solution\\}}}\\
    \bigskip
    Notice Algorithm 1 is basically SIMPLEX.\\
    If we have Algorithm 2, then we can solve any LP.\\
    \begin{itemize}
        \item Using Algo 2 to find a feasible solution, then use Algo 1 to solve it.
    \end{itemize} 
    \textbf{Claim:}\\
    We can use Algo 1 to get Algo 2\\
    \bigskip
    \textbf{First Example:}\\
